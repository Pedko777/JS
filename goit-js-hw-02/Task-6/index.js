...
while (input !== null) { //!ввод не может быть нечем
    input = prompt("Введите число");
    if (isNaN(input)) { //!если введено не число сообщи
      alert("Введено не числопо, пробуйте ещё раз");
    }
  
    if (input !== null && input !== "") { //! если введено значение и введена не пустая строка то в конец масива намберс отправь значение ввода 
      numbers.push(input);
    }
  }
  
  if (numbers.length > 0) { //!если длина масива больше 0 то 
    for (let number of numbers) { //!для числа из чисел
      total = total + Number(number); //! общее значение введенных чисел равняется общей суме 
    }
